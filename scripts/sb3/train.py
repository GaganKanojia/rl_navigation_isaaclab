...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    # resolve custom feature extractor class from string path (if specified in YAML)
    if "policy_kwargs" in agent_cfg:
        pk = agent_cfg["policy_kwargs"]
        if "features_extractor_class" in pk and isinstance(pk["features_extractor_class"], str):
            import importlib

            module_path, class_name = pk["features_extractor_class"].rsplit(".", 1)
            module = importlib.import_module(module_path)
            pk["features_extractor_class"] = getattr(module, class_name)
