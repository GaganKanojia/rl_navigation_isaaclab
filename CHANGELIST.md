...
- **SLAM Toolbox launch argument corrected (`params_file` → `slam_params_file`)**: `online_async_launch.py` declares the parameter file argument as `slam_params_file`, not `params_file`. Passing `params_file:=...` is silently ignored by ROS2 launch and the node falls back to the package's default `mapper_params_online_async.yaml`, ignoring all custom tuning in `slam_toolbox_params.yaml`. Fixed in `README.md` (both SLAM command occurrences), `scripts/ros2_nav2_sim.py` (both modes), and the usage comment in `config/nav2/slam_toolbox_params.yaml`.
- **`use_sim_time` corrected to `true` for Nav2; removed from SLAM commands**: All OmniGraph publishers in `SimBridgeNode` stamp odom, TF, and `/clock` with `ReadSimTime.outputs:simulationTime` — Isaac Sim's physics clock starting from 0 at Play, not wall/epoch time. With `use_sim_time:=false`, Nav2 and SLAM use the system wall clock (~1.7 × 10⁹ s), while the TF buffer only holds entries at sim time (0 → 5 → 10 s). TF lookups fail unconditionally. With `use_sim_time:=true`, all nodes subscribe to `/clock` (also sim time) and perform lookups in the same time domain. SLAM's default is already `true` (`online_async_launch.py` line 34), so `use_sim_time` is simply omitted from SLAM commands. Nav2's default is `false` (`navigation_launch.py` line 87), so it must be explicitly passed as `use_sim_time:=true`.
